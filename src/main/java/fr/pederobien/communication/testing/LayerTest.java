...
import fr.pederobien.communication.impl.layer.*;
...
public class LayerTest {
    ...
    public void testEncapsulaterOneMessage() {
        IExecutable test = () -> {
            Encapsulater encapsulater = new Encapsulater("(~@=", "#.?)");
            String message = "Hello world";

            byte[] toSend = encapsulater.pack(message.getBytes());
            List<byte[]> messages = encapsulater.unpack(toSend);
        ...
        runTest("testEncapsulaterOneMessage", test);
    }

    public void testEncapsulaterTwoMessages() {
        IExecutable test = () -> {
            Encapsulater encapsulater = new Encapsulater("(~@=", "#.?)");
            ...
            byte[] encapsulated1 = encapsulater.pack(message1.getBytes());
            byte[] encapsulated2 = encapsulater.pack(message2.getBytes());
            ...
            List<byte[]> messages = encapsulater.unpack(total);
            for (byte[] received : messages) {
                Logger.info("Unpacked message: %s", new String(received));
            }
        };

        runTest("testEncapsulaterTwoMessages", test);
    }

    public void testEncapsulaterLastMessageTruncated() {
        IExecutable test = () -> {
            Encapsulater encapsulater = new Encapsulater("(~@=", "#.?)");
            String main = "Hello world";
            ...
            List<byte[]> messages = encapsulater.unpack(truncated);
            for (byte[] received : messages) {
                Logger.info("Unpacked message: %s", new String(received));
            }
            ...
            messages = encapsulater.unpack(remaining);
            for (byte[] received : messages) {
                Logger.info("Unpacked message: %s", new String(received));
            }
        };

        runTest("testEncapsulaterLastMessageTruncated", test);
    }
    ...
    public void testSplitterTwoMessages() {
        IExecutable test = () -> {
            ...
            total.addAll(messages1);
            total.addAll(messages2);
