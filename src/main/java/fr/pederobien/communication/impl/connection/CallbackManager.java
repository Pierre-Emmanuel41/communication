...
public class CallbackManager {
	...
	public void register(int identifier, IMessage message) {
		...
		if (message.getCallback().timeout() > 0) {
	...
	/**
	 * If a message is registered for the given identifier, then a separated thread is started to check if a timeout occurs while
	...
	/**
	 * Get the callback associated to the requestID of the response. If no pending request is registered for the requestID then the
	 * method returns null. If a pending request is registered for the requestID it is cancelled to avoid a timeout to occurs.
	 */
	...
	private class Monitor {
		...
		private void monitor() {
			try {
				semaphore.tryAcquire(request.getCallback().timeout(), TimeUnit.MILLISECONDS);
